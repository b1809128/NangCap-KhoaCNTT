...
<body>
    <div class="container">
        <div style="box-shadow: rgba(0, 0, 0, 0.35) 0px 5px 15px; height:100%;">
            ...
            <div class="row" style="margin: 0 10px;">
                ...
                while ($rowNew = mysqli_fetch_array($resultNewForm)) { ?>
                    <div class="col-sm-6" <?php if ($rowNew['Hidden'] === '1') echo "style='display:none;'"; ?>>
                        ...
                        <p style="font-weight:700; text-decoration: underline; color: #0d6efd;">Thành viên hội đồng gồm</p>
                        <?php
                        $json1 = $rowNew['HoiDong'];
                        $obj1 = json_decode($json1, true);
                        foreach ($obj1 as $value) {
                            // $macb = $value;
                            $sqlCheckTeacher = "select * from teacher where MaCB = '$value'";
                            $resultCheckTeacher = mysqli_query($con, $sqlCheckTeacher);
                            $rowCheckTeacher = mysqli_fetch_array($resultCheckTeacher);
                            echo "<p style='font-weight:700;color: #0d6efd;'>".$rowCheckTeacher['HoTen']."</p>";
                        }
                    ...
                    <div style="font-weight:600;" class="col-sm-3">Không đồng ý</div>';
                        ...
                        <div style="padding-left:0px;" class="col-sm-6">';
                            ...
                            while ($row1 = mysqli_fetch_array($resultMaCB)) {
                                ...
                                if ((count($ketQua) > 0) && (count($obj1) > 0)) {
                                    echo $row1['HoTen'] . ' - ' . $ketQua[$i] . '/' . count($obj1) . '(' . round(($ketQua[$i] / count($obj1)) * 100, 2) . '%)';
                                } else {
                                    echo $row1['HoTen'];
                                }
                                echo '</label>';
