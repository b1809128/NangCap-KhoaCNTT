...
if (isset($_POST['submit'])) {
    ...
    $reminderMin = !empty($_POST['reminderMin']) ? trim($_POST['reminderMin']) : '';
    ...
    if (empty($valErr)) {
        ...
        $sqlQ = "INSERT INTO events (title,description,location,date,time_from,time_to,reminders,created) VALUES (?,?,?,?,?,?,?,NOW())";
        $stmt = $db->prepare($sqlQ);
        $stmt->bind_param("sssssss", $db_title, $db_description, $db_location, $db_date, $db_time_from, $db_time_to, $db_reminderMin);
        $db_title = $title;
