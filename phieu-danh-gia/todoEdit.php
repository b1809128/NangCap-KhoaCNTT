...
if (isset($_POST['submitDanhGia'])) {
    ...
    $sqlInsert = "INSERT INTO `phieudanhgiavc` (`idDanhGia`, `idThoiGianDanhGia`, `MaCB`, `Permission`, `BoMon`, `ChinhTriTuTuong`, `DaoDucLoiSong`, `TacPhongLeLoi`, `YThucToChuc`, `KetQuaThucHien`, `ThaiDoPhucVu`, `KetQuaHoatDong`, `NangLucLanhDao`, `TapHopDoanKet`, `UuKhuyetDiem`, `XepLoai`,`NhanXetDanhGiaCapBoMon`,`XepLoaiCapBoMon`,`NhanXetDanhGiaCapKhoa`,`XepLoaiCapKhoa`,`Created_at`)  VALUES (Null,'$idThoiGianDanhGia','$macb','$permission','$bomon','$chinhTriTuTuong','$daoDucLoiSong','$tacPhongLeLoi','$yThucToChuc','$ketQuaThucHien','$thaiDoPhucVu','','','','$uuKhuyetDiem','$xepLoai','','','','',Null) ;";
...
if (isset($_POST['submitDanhGiaCaNhanLanhDao'])) {
    $idDanhGia = $_POST['idDanhGia'];
    $ketQuaHoatDong = $_POST['ketQuaHoatDong'];
    $nangLucLanhDao = $_POST['nangLucLanhDao'];
    $tapHopDoanKet = $_POST['tapHopDoanKet'];
    $sqlUpdate = "UPDATE `phieudanhgiavc` SET `KetQuaHoatDong` = ' $ketQuaHoatDong', `NangLucLanhDao` = '$nangLucLanhDao',`TapHopDoanKet` = '$tapHopDoanKet' WHERE `phieudanhgiavc`.`idDanhGia` = '$idDanhGia';";
    if (mysqli_query($con, $sqlUpdate)) {
        header("Location: http://localhost/joomla/phieu-danh-gia");
    }
}
